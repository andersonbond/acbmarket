  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError(response.data.errors?.[0]?.message || 'Failed to send reset email');
      }
    } catch (err: any) {
      setError(err.response?.data?.errors?.[0]?.message || 'Failed to send reset email. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
